...
source('gamsel/fwd_gamsel_sim.R')
#source('tex_table.R')
source('plots.R')
...
fn.append = paste0(fn.append, '_gamsel')



output.l <- fwd_gamsel_simulation(n, Sigma, groups, num.nonzero, num.linear, lower, upper, nsim, max.steps, design = design, corr = corr)

main.append = paste0("/", round(output.l$spline.fwd.power, 2))
...
ps.fname = paste0(ps.fname, '_gamsel.pdf')
m1 = output.l$m1
...
stop("No grouped variables for splines")
...
output.g <- fwd_gamsel_simulation(n, Sigma, groups, num.nonzero, num.linear, lower, upper, nsim, max.steps, design = design, corr = corr)
