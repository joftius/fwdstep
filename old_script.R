...
bcd_solver = function(X, Y, groups, weights, active.set, lambda, beta, imax, tol=1e-10, maxit=200) {
  continue = TRUE
  iter = 0
  while (continue) {
    iter = iter + 1
    ...
    for (g in active.set) {
      ...
      Xg = X[,inds]
      wg = weights[[g]]
      betaminus = beta * as.numeric(1:length(beta) != inds)
      Sg = t(Xg) %*% (Y - X %*% betaminus)
      betag = max((1 - lambda*wg/norm(Sg, "F")), 0) * Sg
      beta[inds] = betag
    }
    if ((max(abs(betalast - beta)) < tol) | (iter >= maxit)) {
      continue = FALSE
    }
  }
...
sim_forward = function(n=200, g=50, k=5, beta=5, strength=c(1.4, 1.1), maxsteps=20, signaltype="stair",
  ...
  for (iter in 1:nsim) {
    ...
    while (continue) {
      depth = depth + 1
      #(X, Y, groups, weights, active.set=0, residualize=TRUE, alpha=0.1, origX=X, origY=Y, betahat=0)
    ...
    if (compare == TRUE) {
      SGLindex = sort(rep(1:glist, klist))
...
compare = sim_forward(n, g, k, beta, nsim=nsim, strength=strength, compare=FALSE, maxsteps=10)
