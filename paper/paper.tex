...
$\epsilon \sim N(0, \Sigma)$ with known covariance matrix $\Sigma$.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE $P_g \gets I_{n\times n} - X_{g^*}X_{g^*}^\dagger$
    \STATE $A \gets A \cup \{ g^* \}$, $A^c \gets A^c \backslash \{ g^* \}$
    \FORALL{$g \in A^c$}
      \STATE $X_g \gets P_g X_g$
    \ENDFOR
    \STATE $r_s \gets P_g r_{s-1}$
    \ENDFOR
    \RETURN $A$
  \end{algorithmic}
\end{algorithm}
...
set after $k$ steps divided by $k$. (\todo Make this consistent with
...
piecewise linear components. To overcome this difficulty we
