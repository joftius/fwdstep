...
\begin{equation}
  \mu := \mu(X) = \max_{i \neq j} \{ | \innerp{ X_i }{ X_j } | \}
\end{equation}
Recalling that $k$ is the sparsity of $\beta$, it has been shown that if $k < (1/\mu + 1)/2$ and the nonzero coefficients of $\beta$ are sufficiently large then forward stepwise recovers $\beta$ with high probability.
...
But for our purposes the conditions required to guarantee exact recovery are usually too stringent.
...
\hspace{-15pt}
\subfigure[Correlated gaussian design]{
\label{fig:fsgausscorr}
\includegraphics[width=0.5\textwidth]{../figs/tpr_default_gaussian_nsim100_n100_p200_g200_k24_lower1pt2_upper2_corr0pt1.pdf}}
\caption{\small \it The left panel shows the results of the simulation
  ...
  entries with equi-correlation 0.1 between all columns. Plotted in
  green are the TPP and average model sizes for models chosen by BIC.}
\label{fig:fwdstepsim}
\end{center}
\end{figure}

For various sparsity levels $k$, Algorithm~\ref{algo:fs} was run on
various data sets with \textit{steps} equal to $k$.
In the resulting active set $A_k$, the number of true variables divided by $k$ is the True Positive Proportion (TPP):
\[
\text{TPP}_k = \frac{\# \left\{g: g \in A_k, \beta_g \neq 0\right\}}{k}.
\]
Since we know $k$ in the simulations we can use $\text{TPP}_k$ as a measure of how well forward stepwise is performing. When it is close to 1 we are recovering most of the true variables before including false ones.
Nonzero coefficients had magnitudes
...
commonly used model selection procedures, in Figure~\ref{fig:fwdstepsim}
we show in green the TPP and average model sizes for models chosen by
the \textit{step} function in \textit{R} using the BIC criterion.
...
\input{../tables/default_gaussian_nsim500_n100_p50_g50_k5_lower1pt5_upper2.tex}

\input{../tables/default_gaussian_nsim500_n100_p50_g50_k5_lower1pt5_upper2_estimation.tex}
...
\input{../tables/default_gaussian_nsim500_n100_p200_g200_k5_lower1pt5_upper2.tex}
...
\begin{figure}
\begin{center}
\subfigure[Forward stepwise with splines]{
\label{fig:spline:fwd}
\includegraphics[width=0.5\textwidth]{../figs/tpr_gamsel_uniform_nsim50_n100_p200_g200_k21_k01_lower5_upper10.pdf}}
\hspace{-15pt}
\subfigure[P-value with splines]{
\label{fig:spline:pval}
\includegraphics[width=0.5\textwidth]{../figs/gamsel_uniform_nsim100_n200_p50_g50_k6_k02_lower10_upper15.pdf}}
...
\label{fig:glint:fwd}
\includegraphics[width=0.5\textwidth]{../figs/tpr_glint_categorical_nsim50_n100_p72_g36_sizes2-2_k15_k01_lower3_upper5.pdf}}
\hspace{-15pt}
