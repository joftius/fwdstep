...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE $A^c \gets \{ 1, \ldots, G\}$
    \FOR{$s=1$ to $steps$}
    \STATE $g^* \gets \argmax_{g \in A^c} \{ \norm{X_g^T r_{s-1}}_2 / w_g \}$
    \STATE $P \gets I - X_{g^*}X_{g^*}^\dagger$
    \STATE $A \gets A \cup \{ g^* \}$
    \STATE $A^c \gets A^c \backslash \{ g^* \}$
    \FORALL{$g \in A^c$}
...
(lines 8 to 10 above), and since we have weights, we are not computing
...
We now describe how to calculate the p-value, following the discussion of
group lasso in \cite{tests:adaptive}. Let $g = \gstar$ be the index of
the group attaining the maximum on line 4 of Algorithm~\ref{algo:fs},
and let $r = \text{rank}(X_g)$ be the rank of the group attaining the maximum.
