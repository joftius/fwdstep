...
\label{sec:glint}
With many variables, fitting models with pairwise interactions can drastically increase model complexity. \cite{glint} simplify this problem by imposing a hierarchical constraint on interactions (similar to that in \cite{bien:hierarchical}) where an interaction term can only be included if its main effects are also included. They accomplish this by creating a design matrix with main effects alone and also with groups including main effects and their first order interactions. Concretely, the expanded design matrix is
\begin{equation}
\label{eq:glintmat}
...
\]
where * denotes the pointwise product (Hadamard product) of vectors (the $i$th entry of $X_{g1} * X_{h1}$ is the $i$th entry of $X_{g1}$ times the $i$th entry of $X_{h1}$). Note that this expanded matrix has $G(G+1)/2$ groups.
For the rest of this section we drop the tilde
and denote the expanded design matrix as $X$.
...
We can also fit a model by forward stepwise on the same expanded design
matrix. The grouping still guarantees that our fitted model will
satisfy the hierarchy condition. However, we must first modify our
implementation of forward stepwise to prevent the possibility of
including main effect groups after the main effects are already included
in the model as part of an interaction group. To do this we compute the
residual using the whole model in each step, rather than by subtracting
the effect of the current group being included. For a set of group indices
$A \subseteq \{ 1, \ldots G \}$ we write $X_A$ for the submatrix of the
design matrix containing all the groups in $A$.

\begin{algorithm}
\DontPrintSemicolon
\caption{\small \it Modification to compute residual from full model}
\BlankLine
\CommentSty{\# Everything the same as before, except:}\;
