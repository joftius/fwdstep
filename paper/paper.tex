...
\textit{more difficult} for the group $X_g$ to enter the model. The
...
are ready to describe the forward stepwise procedure Algorithm \ref{algo:fs}.
...
\caption{Forward stepwise procedure}
\BlankLine
$A \gets \emptyset$\;
$A^c \gets \{ 1, \ldots, G \}$\;
$r_0 \gets y$\;
\For{$counter \leftarrow 1$ \KwTo $\min (n, G)$}{
...
\begin{itemize}
  ...
  \item Stopping rules, cite new paper on lasso?
\end{itemize}
...
\includegraphics[width=0.5\textwidth]{../figs/small_sim_gaussian_design.pdf}}
\hspace{-15pt}
\subfigure[Categorical design]{
\label{fig:smallcat}
\includegraphics[width=0.5\textwidth]{../figs/small_sim_categorical_design.pdf}}
...
\input{../small_sim_results.tex}

\begin{figure}
\begin{center}
\subfigure[Large Gaussian lasso]{
\label{fig:largelasso}
\includegraphics[width=0.5\textwidth]{../figs/large_sim_gaussian_lasso.pdf}}
\hspace{-15pt}
\subfigure[Large Gaussian group lasso]{
\label{fig:largegrplasso}
\includegraphics[width=0.5\textwidth]{../figs/large_sim_gaussian_design.pdf}}
\caption{\small \it The left panel shows results of a simulation with
  an independent Gaussian design matrix and a signal vector having
  three groups with non-zero coefficients of sizes one, two, and
  three, and seven groups of noise variables with sizes varying from
