...
%    \STATE \COMMENT{Compute the following two $p$-vectors}
    ...
    \STATE Compute solution $(v_-, v_+)$ of \textsc{LinFrac}(a,b) optimization subproblem
    \STATE $u \gets [F_{\chi^2_r}(v^2_-/\sigma_g^2) - F_{\chi^2_r}(\lambda^2/\sigma_g^2)]/[F_{\chi^2_r}(v^2_-/\sigma_g^2) - F_{\chi^2_r}(v^2_+/\sigma_g^2)]$
    \RETURN $u$
  \end{algorithmic}
\end{algorithm}

We next describe the \textsc{LinFrac} subproblem and its solution. We
...
\begin{algorithm}
  \caption{The \textsc{LinFrac} optimization subproblem}
  \label{algo:linfrac}
  \begin{algorithmic}[1]
